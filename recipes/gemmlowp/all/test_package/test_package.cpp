...
std::ostream &operator<<(std::ostream &s, const gemmlowp::MatrixMap<tScalar, tOrder> &m) {
...
int main() {
    ...
    std::array<uint8_t, rows * cols> lhs{1, 2, 3, 4, 5, 6};

    std::array<uint8_t, rows * cols> rhs{7, 8, 9, 10, 11, 12};
    ...
    gemmlowp::Gemm<std::uint8_t, gemmlowp::DefaultL8R8BitDepthParams>(
        &gemm_context, lhs_map, rhs_map, &result_map, 0, 0, 0, 1, 0);
