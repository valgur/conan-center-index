...
class MdnsResponderConan(ConanFile):
    ...
    def validate(self):
        ...
        if Version(self.version) >= "1096.0.2":
            ...
            if self.settings.os == "Windows":
                raise ConanInvalidConfiguration(
                    "Windows is not supported for version {}.".format(self.version)
                )
        ...
        if is_msvc(self) and (
            ...
            or "1910" <= Version(self.settings.compiler.version) <= "1916"
        ):
    ...
    def _make_build_args(self):
        ...
        if (
            ...
            and Version(self.version) != "878.200.35"
            and Version(self.version) != "1310.140.1"
        ):
    ...
    def _install_make(self):
        ...
        rename(
            self,
            os.path.join(self.package_folder, "sbin", "mdnsd"),
    ...
    def _install_msvc(self):
        copy(self, "mDNSResponder.exe",
             dst=os.path.join(self.package_folder, "bin"),
             src=self._msvc_build_folder("mDNSWindows", "SystemService"))
        copy(self, "dns_sd.h",
             dst=os.path.join(self.package_folder, "include"),
             src=os.path.join(self.source_folder, "mDNSShared"))
        copy(self, "dnssd.dll",
             dst=os.path.join(self.package_folder, "bin"),
             src=self._msvc_build_folder("mDNSWindows", "DLL"))
        copy(self, "dnssdStatic.lib",
             dst=os.path.join(self.package_folder, "lib"),
             src=self._msvc_build_folder("mDNSWindows", "DLLStub"))
        # rename consistently with Bonjour SDK
        rename(
            self,
            src=os.path.join(self.package_folder, "lib", "dnssdStatic.lib"),
            dst=os.path.join(self.package_folder, "lib", "dnssd.lib"),
        )
        copy(self, "dns-sd.exe",
             dst=os.path.join(self.package_folder, "bin"),
