...
static void demoCaseMapInC() {
     ...
     */
    static const UChar input[] = {0x61, 0x42, 0x3a3,  0x69,  0x49,  0x131, 0x130, 0x20,
                                  0xdf, 0x20, 0xfb03, 0x3c2, 0x3c3, 0x3a3, 0};
    UChar buffer[32];
    ...
    errorCode = U_ZERO_ERROR;
    length =
        u_strFoldCase(buffer, UPRV_LENGTHOF(buffer), input, -1, U_FOLD_CASE_DEFAULT, &errorCode);
    if (U_SUCCESS(errorCode)) {
    ...
    /* case-fold/Turkic */
    errorCode = U_ZERO_ERROR;
    length = u_strFoldCase(buffer, UPRV_LENGTHOF(buffer), input, -1, U_FOLD_CASE_EXCLUDE_SPECIAL_I,
                           &errorCode);
    if (U_SUCCESS(errorCode)) {
...
static void demoUnicodeStringStorage() {
    ...
    printf("verify that a fastCopyFrom(readonly alias) uses the same buffer pointer: %d (should be "
           "1)\n",
           one.getBuffer() == two.getBuffer());
    // a normal assignment will clone the contents (new in ICU 2.4)
    two = one;
    printf("verify that a regular copy of a readonly alias uses a different buffer pointer: %d "
           "(should be 0)\n",
           one.getBuffer() == two.getBuffer());
...
static void demoUnicodeStringInit() {
    ...
    UnicodeString german =
        UNICODE_STRING("Sch\\u00f6nes Auto: \\u20ac 11240.\\fPrivates Zeichen: \\U00102345\\n", 64)
            .unescape();
    ...
    UChar buffer[200];
    int32_t length;
    length = u_unescape("Sch\\u00f6nes Auto: \\u20ac 11240.\\fPrivates Zeichen: \\U00102345\\n",
