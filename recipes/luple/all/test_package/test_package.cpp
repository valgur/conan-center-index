...
auto get_person2(int id) { return as_nuple($name("key"), std::string{"ivan"}, $name("value"), id); }
...
int main() {
    ...
    printf("get_person2 (as_nuple): %s: %s, %s: %d\n", nuple_ns::name_t<p2_t, 0>::value,
           get<$("key")>(p2).data(), nuple_ns::name_t<p2_t, 1>::value, get<$("value")>(p2));
    ...
    luple_do(p, [](auto &v) { std::cout << v << ": " << typeid(v).name() << "; "; });
