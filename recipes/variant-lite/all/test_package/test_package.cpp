...
ErrorInfo to_int(char const *const text) {
    ...
    return pos == text
               ? ErrorInfo(std::string("Failed to convert '") + text + std::string("' to int"))
               : ErrorInfo(value);
}
...
int main(int argc, char *argv[]) {
    ...
    if (nonstd::get_if<int>(&val))
        std::cout << "'" << text << "' is " << nonstd::get<int>(val) << std::endl;
    else
        std::cout << "'" << text << "' isn't a number. Error: " << nonstd::get<std::string>(val)
                  << std::endl;
}
