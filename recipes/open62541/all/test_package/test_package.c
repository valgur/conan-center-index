...
/* Files namespace_foo_flt_generated.h and namespace_foo_flt_generated.c are created from
 * FooFlt.NodeSet2.xml in the /src_generated directory by CMake */
...
int main(void) {
    ...
    if (namespace_foo_flt_generated(server) != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER,
                     "Could not add the Foo FLT nodeset. "
        ...
        UA_UInt16 nsIdx = UA_Server_addNamespace(
            server, "https://new.foo.com/zebra-compression/flattening-and-subspacefolding/UA/");
