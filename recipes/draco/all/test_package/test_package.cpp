...
int main() {
    ...
    const uint32_t pos_att_id =
        dracoMesh->AddAttribute(pos_attr, true, static_cast<uint32_t>(mesh.vertices.size()));
    for (std::size_t i = 0; i < mesh.vertices.size(); ++i) {
        dracoMesh->attribute(pos_att_id)
            ->SetAttributeValue(draco::AttributeValueIndex(static_cast<uint32_t>(i)),
                                &mesh.vertices[i]);
    }

    for (const auto &primitive : mesh.primitives) {
        dracoMesh->AddFace({draco::PointIndex(primitive.i1), draco::PointIndex(primitive.i2),
