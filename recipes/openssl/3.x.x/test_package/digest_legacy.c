...
int MDx_hash(const EVP_MD *type, const unsigned char *message, size_t message_len,
...
int digest_legacy() {
    ...
    if (MDx_hash(EVP_md5(), (unsigned char *)&string, strlen(string), (unsigned char *)&md5_digest2,
                 &digest_len))
        return 1;
    ...
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++) {
        ...
        snprintf(&md5_string2[i * 2], sizeof(md5_string2) - i * 2, "%02x",
                 (unsigned int)md5_digest2[i]);
    }
    ...
    if (MDx_hash(EVP_md4(), (unsigned char *)&string, strlen(string), (unsigned char *)&md4_digest2,
                 &digest_len)) {
        const char *error_string = ERR_error_string(ERR_get_error(), 0);
    ...
    for (int i = 0; i < MD4_DIGEST_LENGTH; i++) {
        snprintf(&md4_string[i * 2], sizeof(md4_string) - i * 2, "%02x",
                 (unsigned int)md4_digest[i]);
        snprintf(&md4_string2[i * 2], sizeof(md4_string2) - i * 2, "%02x",
                 (unsigned int)md4_digest2[i]);
    }
#endif
    ...
    if (MDx_hash(EVP_ripemd160(), (unsigned char *)&string, strlen(string),
                 (unsigned char *)&ripemd160_digest2, &digest_len)) {
        const char *error_string = ERR_error_string(ERR_get_error(), 0);
    ...
    for (int i = 0; i < RIPEMD160_DIGEST_LENGTH; i++) {
        snprintf(&ripemd160_string[i * 2], sizeof(ripemd160_string) - i * 2, "%02x",
                 (unsigned int)ripemd160_digest[i]);
        snprintf(&ripemd160_string2[i * 2], sizeof(ripemd160_string2) - i * 2, "%02x",
                 (unsigned int)ripemd160_digest2[i]);
    }
#endif
