...
int main(int argc, char const *argv[]) {
    ...
    auto execution_engine = std::unique_ptr<llvm::ExecutionEngine>(engine_builder.create());
    execution_engine->runStaticConstructorsDestructors(false);

    auto test_function = execution_engine->FindFunctionNamed("test");
    auto result =
        execution_engine->runFunction(test_function, llvm::ArrayRef<llvm::GenericValue>());
