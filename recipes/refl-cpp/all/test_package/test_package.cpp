...
template <typename T> constexpr bool checkNonNegative(const T &obj) {
    ...
    // Filter out the non-readable members (not field or getters marked with the property()
    // attribute)
    ...
    auto invalidMemberCount = count_if(readableMembers, [&](auto member) {
        // Function-call syntax is a uniform way to get the value of a member (whether a field or a
        // getter)
