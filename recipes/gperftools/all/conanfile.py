...
class GperftoolsConan(ConanFile):
    ...
    def _msbuild_configuration(self):
        # TODO: add option to select override vs patch to
        # "allow users to choose to override malloc/free/etc on Windows rather than patching.
        # This can be used to create custom CRTs."
        return  "Debug" if self.settings.build_type == "Debug" else "Release-Patch"
    ...
    def generate(self):
        ...
        if is_msvc(self):
            ...
            # TODO: apply options
            # TODO: disable building of executables and unit tests
            tc.generate()
            ...
            for vcxproj_file in (self.source_path / "vsprojects").rglob("*.vcxproj"):
            ...
            if self._build_minimal:
                ...
                args["enable-stacktrace-via-backtrace"] = self.options.get_safe("enable_stacktrace_via_backtrace", False)
    ...
    def build(self):
        if is_msvc(self):
            ...
            msbuild.build(os.path.join(self.source_folder, "gperftools.sln"))
        else:
            self._patch_sources()
    ...
    def package(self):
        ...
        if is_msvc(self):
            copy(self, "*.h",
                 ...
                 src=os.path.join(self.source_folder, "src", "google"),
                 keep_path=False)
            # FIXME: tcmalloc.h needs to be generated from cmake/tcmalloc.h.in
            copy(self, "tcmalloc.h",
                 dst=os.path.join(self.package_folder, "include", "gperftools"),
                 src=os.path.join(self.source_folder, "src", "gperftools"),
                 keep_path=False)
            for path in self.source_path.rglob("*.lib"):
                if "test" not in path.name:
                    copy(self, path.name, dst=self.package_path / "lib", src=path.parent)
            for path in self.source_path.rglob("*.dll"):
                if "test" not in path.name:
                    copy(self, path.name, dst=self.package_path / "bin", src=path.parent)
        else:
            autotools = Autotools(self)
            autotools.install()
    ...
    def _add_component(self, lib):
        ...
        self.cpp_info.components[lib].libs = [f"{prefix}{lib}"]
