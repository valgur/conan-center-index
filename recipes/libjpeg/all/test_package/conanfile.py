...
class TestPackageConan(ConanFile):
    ...
    def _build_transupp(self):
        ...
        return not (
            self.dependencies["libjpeg"].options.shared and is_msvc(self) and is_msvc_static_runtime(self)
        )
    ...
    def generate(self):
        ...
        if self._build_transupp:
            tc.variables["LIBJPEG_RES_DIR"] = (
                self.dependencies["libjpeg"].cpp_info.resdirs[0].replace("\\", "/")
            )
        tc.generate()
