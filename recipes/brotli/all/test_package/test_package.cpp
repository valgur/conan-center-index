...
int main() {
    ...
    BROTLI_BOOL result = BrotliEncoderCompress(
        BROTLI_DEFAULT_QUALITY, BROTLI_DEFAULT_WINDOW, BROTLI_DEFAULT_MODE, input.size(),
        reinterpret_cast<const uint8_t *>(input.c_str()), &outputsize, buffer);
    ...
    result = BrotliDecoderDecompress(outputsize, buffer, &reconstructedSize,
    ...
    if (reconstructedSize != input.size()) {
        std::cout << "Size of input (" << input.size() << ") and output (" << reconstructedSize
                  << ") do not match\n";
        return EXIT_FAILURE;
    }
