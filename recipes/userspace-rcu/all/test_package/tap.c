...
#ifdef __MINGW32__
static inline void flockfile(FILE *filehandle) { return; }

static inline void funlockfile(FILE *filehandle) { return; }
#endif
 ...
 */
unsigned int _gen_result(int ok, const char *func, const char *file, unsigned int line,
...
void _cleanup(void) {
    ...
    if ((have_plan && !no_plan) && e_tests < test_count) {
        diag("Looks like you planned %d %s but ran %d extra.", e_tests,
             e_tests == 1 ? "test" : "tests", test_count - e_tests);
        UNLOCK;
        return;
    }

    if ((have_plan || !no_plan) && e_tests > test_count) {
        diag("Looks like you planned %d %s but only ran %d.", e_tests,
             e_tests == 1 ? "test" : "tests", test_count);
        UNLOCK;
        return;
    }

    if (failures)
        diag("Looks like you failed %d %s of %d.", failures, failures == 1 ? "test" : "tests",
             test_count);
