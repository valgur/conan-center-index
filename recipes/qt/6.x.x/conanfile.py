...
class QtConan(ConanFile):
    ...
    def _get_module_tree(self):
        ...
        for m in self._submodules_tree:
            ...
            ), f"module {m} not in self._submodules"
    ...
    def validate(self):
        if os.getenv("NOT_ON_C3I", "0") == "0":
            if (
                ...
                and Version(self.info.settings.compiler.version) >= "11"
                or self.info.settings.compiler == "clang"
        ...
        if self.settings.compiler == "clang" and "libstdc++" in str(self.settings.compiler.libcxx):
            raise ConanInvalidConfiguration(
                "Qt needs recent libstdc++, with charconv. please switch to gcc, or to libc++"
            )
    ...
    def generate(self):
        ...
        for f in glob.glob("*.cmake"):
            replace_in_file(self, f, " IMPORTED)\n", " IMPORTED GLOBAL)\n", strict=False)
        ...
        if self.settings.os == "Windows":
            tc.variables["HOST_PERL"] = getattr(self, "user_info_build", self.deps_user_info)[
                "strawberryperl"
            ].perl
    ...
    def package(self):
        ...
        for target in targets:
            exe_path = None
            for path_ in [f"bin/{target}{extension}", f"lib/{target}{extension}"]:
    ...
    def package_info(self):
        ...
        self.runenv_info.define(
            "QT_PLUGIN_PATH", os.path.join(self.package_folder, "res", "archdatadir", "plugins")
        )
        ...
        def _create_plugin(pluginname, libname, plugintype, requires):
            ...
            self.cpp_info.components[componentname].libdirs = [
                os.path.join("res", "archdatadir", "plugins", plugintype)
            ]
            self.cpp_info.components[componentname].includedirs = []
        ...
        self.cpp_info.components["qtPlatform"].includedirs = [
            os.path.join("res", "archdatadir", "mkspecs", self._xplatform())
        ]
        if self.options.with_dbus:
        ...
        if self.settings.os in ["Windows", "iOS"]:
            if self.settings.os == "Windows":
                ...
                if self.settings.compiler == "gcc":
                    ...
                    self.cpp_info.components["qtEntryPointMinGW32"].names[
                        "cmake_find_package"
                    ] = "EntryPointMinGW32"
                    self.cpp_info.components["qtEntryPointMinGW32"].names[
                        "cmake_find_package_multi"
