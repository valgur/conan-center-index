...
int main() {
    ...
    std::cerr << "Number of enumerators: " << wise_enum::enumerators<my_lib::Color>::size << "\n";
    ...
    static_assert(std::is_same<int64_t, typename std::underlying_type<MoreColor>::type>::value, "");

    // Full API available for adapted wise enums
    for (auto e : wise_enum::enumerators<another_lib::SomebodyElse>::range) {
        std::cerr << static_cast<int>(e.value) << " " << wise_enum::to_string(e.value) << "\n";
    }
