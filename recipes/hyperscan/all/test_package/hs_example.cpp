...
static std::tuple<std::shared_ptr<hs_database_t>, std::shared_ptr<hs_compile_error_t>, hs_error_t>
compile(const char *pattern) {
    hs_compile_error_t *err = nullptr;
    hs_database_t *db = nullptr;

    const auto result =
        hs_compile(pattern, HS_FLAG_SOM_LEFTMOST, HS_MODE_BLOCK, nullptr, &db, &err);

    return std::tuple<std::shared_ptr<hs_database_t>, std::shared_ptr<hs_compile_error_t>,
                      hs_error_t>(std::shared_ptr<hs_database_t>(db, hs_free_database),
...
static std::pair<std::shared_ptr<hs_scratch_t>, hs_error_t> alloc_scratch(hs_database_t &db) {
    ...
    return std::pair<std::shared_ptr<hs_scratch_t>, hs_error_t>(
        std::shared_ptr<hs_scratch_t>(scratch, hs_free_scratch), result);
}

static hs_error_t scan(hs_database_t &db, hs_scratch_t &scratch, const char *data,
