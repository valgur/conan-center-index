...
void thread(boost::fibers::detail::thread_barrier *b) {
    ...
    boost::fibers::use_scheduling_algorithm<
        boost::fibers::algo::shared_work>(); /**<
                                              ...
                                              `boost::fibers::algo::shared_work` in order to join
                                              the work sharing.
                                              >*/
...
int main(int argc, char *argv[]) {
    ...
    boost::fibers::use_scheduling_algorithm<
        boost::fibers::algo::shared_work>(); /*<
                                              Install the scheduling algorithm
                                              `boost::fibers::algo::shared_work` in the main thread
                                              too, so each new fiber gets launched into the shared
                                              pool.
                                              >*/

    for (char c :
         std::string("abcdefghijklmnopqrstuvwxyz")) { /*<
                                                       Launch a number of worker fibers; each worker
                                                       fiber picks up a character that is passed as
                                                       parameter to fiber-function `whatevah`. Each
                                                       worker fiber gets detached.
                                                       >*/
    ...
    std::thread threads[] = {/*<
                              ...
                              >*/
                             std::thread(thread, &b), std::thread(thread, &b),
                             std::thread(thread, &b)};
    b.wait(); /*< sync with other threads: allow them to start processing >*/
    {
