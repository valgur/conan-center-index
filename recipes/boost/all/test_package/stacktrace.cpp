...
static const char *stacktrace_impls[] = {
    "addr2line", "backtrace", "basic", "noop", "windbg", "windbg_cached",
};
...
bool check_used_defined() {
    ...
    std::cerr
        << "testing stacktrace_windbg_cached but BOOST_STACKTRACE_USE_WINDBG_CACHED not defined\n";
    success = false;
#endif
#endif
    return success;
...
#if TEST_STACKTRACE_IMPL != TEST_STACKTRACE_WINDBG_CACHED
    std::cerr
        << "BOOST_STACKTRACE_USE_WINDBG_CACHED defined but not testing stacktrace_windbg_cached\n";
    success = false;
#endif
#endif
    return success;
