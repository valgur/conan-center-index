...
static void for_each_provider(const char *const provider_name, const char *const provider_desc,
                              const char *const provider_dll_file, void *user_data) {
    (void)provider_desc;
...
int main(void) {
    ...
    if (counter != 1) {
        (void)fflush(stdout);
        (void)fprintf(stderr, "Expected to find exactly 1 provider, found %d\n", counter);
        return_code = 1;
    } else {
        (void)printf("\nenchant version %s\n", enchant_get_version());
    }
