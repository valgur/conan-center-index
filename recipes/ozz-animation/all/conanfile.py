...
class OzzAnimationConan(ConanFile):
    ...
    def source(self):
        ...
        if Version(self.version) < "0.14.2":
            ...
            compiler_settings = Path(self.source_folder) / "build-utils" / "cmake" / "compiler_settings.cmake"
        ...
        replace_in_file(self, Path(self.source_folder) / "src" / "animation" / "offline" / "tools" / "CMakeLists.txt",
                        "if(NOT EMSCRIPTEN)",
    ...
    def package(self):
        copy(self, "LICENSE.md", self.source_folder, Path(self.source_folder, "licenses"))
        cmake = CMake(self)
        cmake.install()

        if self.options.ozz_animation_tools:
            json_dir = Path(self.build_folder).joinpath("src", "animation", "offline", "tools", "json")
            for pattern in ["*.a", "*.so*", "*.dylib", "*.lib"]:
                copy(self, pattern, src=json_dir, dst=Path(self.package_folder, "lib"), keep_path=False)
            copy(self, "*.dll", src=json_dir, dst=Path(self.package_folder, "bin"), keep_path=False)

        mkdir(self, Path(self.package_folder, "bin"))
        for file in Path(self.package_folder, "lib").glob("*.dll"):
            rename(self, src=file, dst=Path(self.package_folder, "bin", file.name))

        if self.options.tools:
            for file in Path(self.package_folder, "bin", "tools").iterdir():
                rename(self, src=file, dst=Path(self.package_folder, "bin", file.name))
            rmdir(self, Path(self.package_folder, "bin", "tools"))

        rm(self, "*.md", self.package_folder)
