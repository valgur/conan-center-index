...
ErrorInfo to_int(char const *const text) {
    ...
    return pos == text ? nonstd::make_unexpected(std::string("Failed to convert '") + text +
                                                 std::string("' to int"))
                       : ErrorInfo(value);
}
...
int main(int argc, char *argv[]) {
    ...
    if (val)
        std::cout << "'" << text << "' is " << val.value() << std::endl;
    else
        std::cout << "'" << text << "' isn't a number. Error: " << val.error() << std::endl;
}
