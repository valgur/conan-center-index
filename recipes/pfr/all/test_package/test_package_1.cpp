...
static_assert(std::is_same<boost::pfr::flat_tuple_element_t<1, foo_t>, // Flat reflection.
...
static_assert(std::is_same<boost::pfr::tuple_element_t<1, foo_t>, // Precise reflection.
                           nested_t>::value,
              ""); //<-
#endif             //->
//] [/pfr_intro]
...
//[pfr_example_get
/*`
    The following example shows how to access structure fields by index using [funcref
   boost::pfr::get].
...
foo f{777, '!'};
auto &r1 =
    boost::pfr::get<0>(f); // accessing field with index 0, returns reference to `foo::some_integer`
...
static_assert(boost::pfr::tuple_size<foo2>::value // returns total count of fields in `foo2`
                  == 3,
              "");

static_assert(boost::pfr::tuple_size<int[100]>::value // works with arrays too!
                  == 100,
              "");
//] [/pfr_example_tuple_size]
...
/*`
[warning All the functions and metafunctions in Boost.PFR that start with `flat_` represent template
parameter type as flat structure without static members! ]

Take a look at the `struct my_struct`:
*/
#include <boost/pfr/flat/core.hpp>

struct my_struct_nested {
    short a1;
    int a2;
};
...
void example_get() {
    ...
    It means, that:
    */
    boost::pfr::flat_get<2>(
        my_struct{}); // ... will return `my_struct::my_struct_nested::a2` field.
