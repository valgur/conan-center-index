...
void BlobInfo_Print(const uint32 *infoArr) {
    ...
    printf("version = %d, dataType = %d, nDim = %d, nCols = %d, nRows = %d, nBands = %d, "
           "nValidPixels = %d, blobSize = %d\n",
           ia[0], ia[1], ia[2], ia[3], ia[4], ia[5], ia[6], ia[7]);
}

bool BlobInfo_Equal(const uint32 *infoArr, uint32 nDim, uint32 nCols, uint32 nRows, uint32 nBands,
                    uint32 dataType) {
    const uint32 *ia = infoArr;
    return ia[1] == dataType && ia[2] == nDim && ia[3] == nCols && ia[4] == nRows &&
           ia[5] == nBands;
}
...
void sample1() {
    ...
    lerc_status hr = lerc_computeCompressedSize(
        reinterpret_cast<const void *>(&zImg[0]), // raw image data, row by row, band by band
...
#ifdef LERC_VER3_LATER
    hr = lerc_decode(&pLercBlob[0], numBytesBlob, 1, maskByteImg3.data(), 1, w, h, 1,
                     static_cast<uint32>(dt_float), reinterpret_cast<void *>(&zImg3[0]));
#else
    hr = lerc_decode(&pLercBlob[0], numBytesBlob, &maskByteImg3[0], 1, w, h, 1,
                     static_cast<uint32>(dt_float), reinterpret_cast<void *>(&zImg3[0]));
#endif
    if (hr)
    ...
    for (int k = 0, i = 0; i < h; i++) {
        for (int j = 0; j < w; j++, k++) {
            if (maskByteImg3[k] != maskByteImg[k])
                std::cout << "Error in main: decoded valid bytes differ from encoded valid bytes"
                          << std::endl;
