...
#define double_to_color(x) (((uint32_t)((x)*65536)) - (((uint32_t)((x)*65536)) >> 16))

#define PIXMAN_STOP(offset, r, g, b, a)                                                            \
    {                                                                                              \
        pixman_double_to_fixed(offset), {                                                          \
            double_to_color(r), double_to_color(g), double_to_color(b), double_to_color(a)         \
        }                                                                                          \
    }

static const pixman_gradient_stop_t stops[] = {
    PIXMAN_STOP(0.25, 1, 0, 0, 0.7), PIXMAN_STOP(0.5, 1, 1, 0, 0.7),
    PIXMAN_STOP(0.75, 0, 1, 0, 0.7), PIXMAN_STOP(1.0, 0, 0, 1, 0.7)};
...
int main() {
    ...
    src_img =
        pixman_image_create_conical_gradient(&c, pixman_double_to_fixed(angle), stops, NUM_STOPS);
