...
/// enabling/disabling respective reactor framework notifications.
{
  public:
    EchoServiceHandler(StreamSocket &socket, SocketReactor &reactor)
        : _socket(socket), _reactor(reactor), _fifoIn(BUFFER_SIZE, true),
        ...
        _reactor.addEventHandler(_socket, NObserver<EchoServiceHandler, ShutdownNotification>(
    ...
    ~EchoServiceHandler() {
        ...
        _reactor.removeEventHandler(_socket, NObserver<EchoServiceHandler, ShutdownNotification>(
                                                 *this, &EchoServiceHandler::onSocketShutdown));
    ...
    void onSocketWritable(const AutoPtr<WritableNotification> &pNf) { _socket.sendBytes(_fifoOut); }

    void onSocketShutdown(const AutoPtr<ShutdownNotification> &pNf) { delete this; }
  ...
  protected:
    ...
    void displayHelp() {
        ...
        helpFormatter.setHeader(
            "An echo server implemented using the Reactor and Acceptor patterns.");
