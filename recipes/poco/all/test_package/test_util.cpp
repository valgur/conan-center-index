...
/// more information.
{
  ...
  protected:
    ...
    void defineOptions(OptionSet &options) {
        ...
        options.addOption(Option("define", "D", "define a configuration property")
                              .required(false)
                              .repeatable(true)
                              .argument("name=value")
                              .callback(OptionCallback<SampleApp>(this, &SampleApp::handleDefine)));

        options.addOption(Option("config-file", "f", "load configuration data from a file")
                              .required(false)
                              .repeatable(true)
                              .argument("file")
                              .callback(OptionCallback<SampleApp>(this, &SampleApp::handleConfig)));

        options.addOption(Option("bind", "b", "bind option value to test.property")
                              .required(false)
                              .repeatable(false)
                              .argument("value")
    ...
    void displayHelp() {
        ...
        helpFormatter.setHeader("A sample application that demonstrates some of the features of "
                                "the Poco::Util::Application class.");
    ...
    void printProperties(const std::string &base) {
        ...
        if (keys.empty()) {
            ...
            for (AbstractConfiguration::Keys::const_iterator it = keys.begin(); it != keys.end();
                 ++it) {
