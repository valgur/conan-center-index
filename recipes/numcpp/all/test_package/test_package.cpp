...
using FunctionType =
    std::function<double(const nc::NdArray<double> &, const nc::NdArray<double> &)>;

void wikipediaExample() {
    // https://en.wikipedia.org/wiki/Gauss%E2%80%93Newton_algorithm

    // In a biology experiment studying the relation between substrate concentration [S] and
    // reaction rate in an enzyme-mediated reaction, the data in the following table were obtained.
    ...
    FunctionType function = [](const nc::NdArray<double> &coordinates,
    ...
    FunctionType delFdelBeta1 = [](const nc::NdArray<double> &coordinates,
                                   const nc::NdArray<double> &betas) -> double {
        const double s = coordinates.at(0);
        const double beta2 = betas.at(1);
    ...
    FunctionType delFdelBeta2 = [](const nc::NdArray<double> &coordinates,
                                   const nc::NdArray<double> &betas) -> double {
        const double s = coordinates.at(0);
        const double beta1 = betas.at(0);
        const double beta2 = betas.at(1);
    ...
    // starting with the initial estimates of beta1Guess and beta2Guess and calculating after 5
    // iterations
    ...
    auto [betas, rms] =
        nc::linalg::gaussNewtonNlls(numIterations, sMeasured.transpose(), rateMeasured, function,
                                    {delFdelBeta1, delFdelBeta2}, beta1Guess, beta2Guess);
#else
    auto results =
        nc::linalg::gaussNewtonNlls(numIterations, sMeasured.transpose(), rateMeasured, function,
                                    {delFdelBeta1, delFdelBeta2}, beta1Guess, beta2Guess);
