...
void findDelaunay(qhT *qh, int dim) {
    ...
    if (facet->tricoplanar) {
        fprintf(stderr,
                "findDelaunay: search not implemented for triangulated, non-simplicial Delaunay "
                "regions (tricoplanar facet, f%d).\n",
                facet->id);
...
\n");
    ...
    */
    ...
    if (curlong || totlong)
        fprintf(errfile,
                "qhull internal warning (user_eg, #1): did not free %d bytes of long memory (%d "
                "pieces)\n",
                totlong, curlong);
#endif
    ...
    */
    ...
    }
    {
        ...
        fflush(NULL);
        exitcode = qh_new_qhull(qhB, dim, numpoints, pointsB, ismalloc, flags, outfile, errfile);
        fflush(NULL);
        if (!exitcode)
            print_summary(qhB);
        printf("\n========\nFree memory allocated by the new instance of Qhull, and redisplay the "
               "old results.\n");
#ifdef qh_NOmem
        qh_freeqhull(qh, qh_ALL);
#else
        ...
        if (curlong || totlong)
            fprintf(errfile,
                    "qhull internal warning (user_eg, #4): did not free %d bytes of long memory "
                    "(%d pieces)\n",
                    totlong, curlong);
#endif
        printf("\n\n");
    ...
    if (curlong || totlong)
        fprintf(errfile,
                "qhull internal warning (user_eg, #2): did not free %d bytes of long memory (%d "
                "pieces)\n",
                totlong, curlong);
#endif
    ...
    */
    ...
    qh_printmatrix(qh, outfile, "input as halfspace coefficients + offsets", rows, numpoints,
                   dim + 1);
    fflush(NULL);
    /* use qh_sethalfspace_all to transform the halfspaces yourself.
       If so, set 'qh->feasible_point and do not use option 'Hn,...' [it would retransform the
       halfspaces]
    */
    exitcode = qh_new_qhull(qh, dim + 1, numpoints, points, ismalloc, flags, outfile, errfile);
    fflush(NULL);
    if (!exitcode)
        print_summary(qh);
#ifdef qh_NOmem
    ...
    if (curlong || totlong) /* could also check previous runs */
        fprintf(stderr,
                "qhull internal warning (user_eg, #3): did not free %d bytes of long memory (%d "
                "pieces)\n",
                totlong, curlong);
#endif
    return exitcode;
